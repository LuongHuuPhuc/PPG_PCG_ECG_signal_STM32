...
void max30102_set_a_full_itrp(max30102_t *obj, uint8_t enable){
    ...
    /* Doc gia tri thanh ghi INTERRUPT_ENABLE_1 de lay trang thai On-Off hien tai */
    max30102_read(obj, MAX30102_INTERRUPT_ENABLE_1, &reg, 1);

    /* Clear bit */
    reg &= ~(0x01 << MAX30102_INTERRUPT_A_FULL_EN);
    reg |= ((enable & 0x01) << MAX30102_INTERRUPT_A_FULL_EN);

    /* Ghi lai gia tri vao thanh ghi */
...
void max30102_set_ppg_rdy_itrp(max30102_t *obj, uint8_t enable){
    uint8_t reg = 0;
    max30102_read(obj, MAX30102_INTERRUPT_ENABLE_1, &reg, 1);
    reg &= ~(0x01 << MAX30102_INTERRUPT_PPG_RDY_EN);
    reg |= ((enable & 0x01) << MAX30102_INTERRUPT_PPG_RDY_EN);
    max30102_write(obj, MAX30102_INTERRUPT_ENABLE_1, &reg, 1);
}
...
void max30102_set_alc_ovf_itrp(max30102_t *obj, uint8_t enable){
    uint8_t reg = 0;
    max30102_read(obj, MAX30102_INTERRUPT_ENABLE_1, &reg, 1);
    reg &= ~(0x01 << MAX30102_INTERRUPT_ALC_OVF_EN);
    reg |= ((enable & 0x01) << MAX30102_INTERRUPT_ALC_OVF_EN);
    max30102_write(obj, MAX30102_INTERRUPT_ENABLE_1, &reg, 1);
}
...
void max30102_set_power_ready(max30102_t *obj, uint8_t enable){
	uint8_t reg = (enable & 0x01) << MAX30102_INTERRUPT_PWR_RDY;
	max30102_write(obj, MAX30102_INTERRUPT_STATUS_1, &reg, 1);
}

/*-----------------------------------------------------------*/

void max30102_set_die_temp_rdy_itrp(max30102_t *obj, uint8_t enable){
    uint8_t reg = (enable & 0x01) << MAX30102_INTERRUPT_DIE_TEMP_RDY_EN;
...
void max30102_interrupt_handler(max30102_t *obj, max30102_record *record){
    ...
    // Bang cach doc thanh ghi Interrupt Status 1-2 (0x00 va 0x01) -> tat ca cac bit se duoc clear/reset
    max30102_read(obj, MAX30102_INTERRUPT_STATUS_1, reg, 2);

    if ((reg[0] >> MAX30102_INTERRUPT_A_FULL) & 0x01){
        // FIFO gan day thi doc FIFO
    ...
    if ((reg[1] >> MAX30102_INTERRUPT_DIE_TEMP_RDY) & 0x01){
        ...
        obj->sensor_temp = temp_int + 0.0625f * temp_frac;
    }
...
uint16_t max30102_read_fifo(max30102_t *obj, max30102_record *record, uint16_t max_samples){
	...
	/* Thuc hien doc con tro de tinh toan so samples hien co */
#ifdef DEBUG_SWV_ITM
...
#ifdef DEBUG_SWV_ITM
			/* Doc lai rd_ptr sau khi I2C read xong FIFO de biet thuc su da doc duoc bao nhieu (theo doi con tro doc)*/
			uint8_t rd_ptr_after = max30102_get_read_pointer(obj);

			/**
			 * So con tro doc truoc va sau khi doc FIFO de xem con tro doc dich bao nhieu tinh so samples thuc su da doc duoc.
			 * Binh thuong, neu doc du 32 samples, con tro doc se quay lai 1 vong ve vi tri cu. Nhung neu co ly do nao do
			 * khien con tro doc bi lech (vd: rd_ptr_after lai be hon rd_ptr_before hoac bi ket khien no khong quay het vong FIFO)
			 */
	...
	/* Note: Must read the FIFO_samples_debuglog.txt to see how hard bug is TT */
}
